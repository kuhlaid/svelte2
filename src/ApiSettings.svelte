...
*/
    ...
    function getApiToken() {
        storeApiTokenReadable = readable(localStorage.getItem("strApiToken") || "");
        const unsubscribeReable = storeApiTokenReadable.subscribe(value => {
            strApiToken = value;
        });
        console.log("getApiToken");
        console.log($storeApiTokenReadable);
    }
...
<button on:click={btnGetApiDataClick} class="btn btn-primary" disabled={!$storeApiUrlReadable}>Get ProduceFormly data from the server API</button>
